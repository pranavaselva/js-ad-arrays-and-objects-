...
const lens = strings.filter(function (item) {
...
const add = alphabets.reduce(function (result, item) {
  return result + item;
});
console.log(add);
...
const fin = ranks.findIndex(function (item) {
  return item == 7;
});
console.log(fin);
...
//3:8
const num = [2, 4, 6, 8];
const nin = num.forEach(function (item, index) {
...
const less = marks.some(function (item) {
  return item > 45;
});
console.log(less);
...
const alpos = positive.every(function (item) {
  return item > 0;
});
console.log(alpos);
...
console.log(cars[2].name, cars[2].color[2]);
